...
function MyResumes() {
  ...
  useEffect(() => {
    ...
    if (refreshParam) {
      console.log('Force refreshing resumes due to recent improvement');
  ...
  }, [location.state, location.search, currentPage, searchTerm]);
  ...
  const improveResume = (resume) => {
    setSelectedResume(resume);
    setShowImprover(true);
  };

  const handleResumeImproved = (newResume) => {
    // Refresh the resumes list to show the new improved resume
    loadResumes();
    setShowSuccess(true);
    setSuccessMessage(`Improved resume "${newResume.title}" has been created successfully!`);
    setTimeout(() => setShowSuccess(false), 5000);
  };
  ...
  return (
    <div className="container mt-4">
      ...
      {showImprover && selectedResume && (
        <ResumeImprover
          resume={selectedResume}
          jobDescription={jobDescription}
          onClose={closeImprover}
          onImproved={handleResumeImproved}
        />
      )}
    </div>
  );
